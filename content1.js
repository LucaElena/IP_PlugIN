...
function gotMessage(message, sender , sendResponse)
{
	...
	if(message.txt === "give_me_current_postdata_facebook")
	{
		...
		for(var i = divs.length - 1; i >= 0; i--)
		{
			...
			if(divs[i].className == "jm1wdb64 19j0dhe7 datstx6m k4urcfbm")
			{
		...
		for(var i = divs.length - 1; i >= 0; i--)
		{
			...
			if(divs[i].className == "photoset")
			{
				...
				current_postdata.data.post_imgs.push( divs[i].src);
                        ...
                        if(divs[i].className == "class="crt-video crt-skin-default vjs-paused vjs_video_3-dimensions vjs-controls-enabled vjs-workinghover vjs-user-inactive")
			{
				console.log('Found tumblr post video='+ divs[i].src);
				current_postdata.txt = "current_postdata_defined_tumblr";
				current_postdata.data.post_video.push( divs[i].src);
			}
		}
		// current_postdata.txt = "current_postdata_defined_tumblr";
	...
	if(message.txt === "give_me_current_postdata_linkedin")
	{
		...
		for(var i = divs.length - 1; i >= 0; i--)
		{
			...
			if(divs[i].className == "video-js media-player__player vjs-paused vjs-fluid vjs-controls-enabled vjs-workinghover vjs-v7 vjs-user-active vjs-layout-medium")
			{
				console.log('Found linkedin post video='+ divs[i].getElementsByTagName('video')[0].src);
				current_postdata.txt = "current_postdata_defined_linkedin";
				current_postdata.data.post_videos.push( divs[i].getElementsByTagName('video')[1].src);
			}
		}
